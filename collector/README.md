# Collector Service ðŸ“¥

Collector module for H429 data import and SQLite sync âš™ï¸

## 1. What This Folder Contains ðŸ“
...
- `csv_received/` â†’ incoming files waiting to import
- `csv_archived/` â†’ processed files after successful import
...
This is used by the frontend/backend `DATA CONNECTION` control on the home page ðŸ§­
...
```bash
cd collector
python mode2_import_data_from_drums.py --once
```
...
```bash
cd collector
python mode2_import_data_from_drums.py
```
...
```bash
cd collector
python mode1_data_collector_from_database.py --once
```
...
```bash
cd collector
python mode1_data_collector_from_database.py
```
...
## 11. Troubleshooting ðŸ› ï¸

1. No data in frontend âŒ
...
- `mode2_import_data_from_drums.py` is the current replacement for the older CSV import flow
